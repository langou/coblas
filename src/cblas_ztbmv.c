...
void cblas_ztbmv(CBLAS_ORDER order, CBLAS_UPLO uplo, CBLAS_TRANSPOSE trans, CBLAS_DIAG diag, int n, int k, double complex *A, int ldA, double complex *x, int incx)
{
    if((order!=CblasRowMajor)&&(order!=CblasColMajor))
        cblas_xerbla(1,"cblas_ztbmv","");
    else if((uplo!=CblasUpper)&&(uplo!=CblasLower))
        cblas_xerbla(2,"cblas_ztbmv","");
    else if((trans!=CblasNoTrans)&&(trans!=CblasTrans)&&(trans!=CblasConjTrans))
        cblas_xerbla(3,"cblas_ztbmv","");
    else if((diag!=CblasUnit)&&(diag!=CblasNonUnit))
        cblas_xerbla(4,"cblas_ztbmv","");
    else if(n<0)
        cblas_xerbla(5,"cblas_ztbmv","");
    else if(k<0)
        cblas_xerbla(6,"cblas_ztbmv","");
    else if(ldA<k+1)
        cblas_xerbla(8,"cblas_ztbmv","");
    else if(incx==0)
        cblas_xerbla(10,"cblas_ztbmv","");
    else
    {
            ...
            else if(trans==CblasConjTrans)
            {
                if(uplo==CblasUpper)
                {
                    ...
                    for(int j=n-1;j>=0;j--)
                    {
                        ...
                        int i0=maxsub(j,k);
                        for(int i=j-1;i>=i0;i--)
                            x[j*incx]+=conj(A[k+i-j])*x[i*incx];
                    }
                }
                else if(uplo==CblasLower)
                {
