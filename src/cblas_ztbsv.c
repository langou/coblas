...
void cblas_ztbsv(CBLAS_ORDER order, CBLAS_UPLO uplo, CBLAS_TRANSPOSE trans, CBLAS_DIAG diag, int n, int k, double complex *A, int ldA, double complex *x, int incx)
{
    if((order!=CblasRowMajor)&&(order!=CblasColMajor))
        cblas_xerbla(1,"cblas_ztbsv","");
    else if((uplo!=CblasUpper)&&(uplo!=CblasLower))
        cblas_xerbla(2,"cblas_ztbsv","");
    else if((trans!=CblasNoTrans)&&(trans!=CblasTrans)&&(trans!=CblasConjTrans))
        cblas_xerbla(3,"cblas_ztbsv","");
    else if((diag!=CblasUnit)&&(diag!=CblasNonUnit))
        cblas_xerbla(4,"cblas_ztbsv","");
    else if(n<0)
        cblas_xerbla(5,"cblas_ztbsv","");
    else if(k<0)
        cblas_xerbla(6,"cblas_ztbsv","");
    else if(ldA<k+1)
        cblas_xerbla(8,"cblas_ztbsv","");
    else if(incx==0)
        cblas_xerbla(10,"cblas_ztbsv","");
    else
    {
            ...
            else if(trans==CblasConjTrans)
            {
                if(uplo==CblasUpper)
                {
                    for(int j=0;j<n;j++)
                    {
                        int i0=maxsub(j,k);
                        for(int i=i0;i<j;i++)
                            x[j*incx]-=x[i*incx]*conj(A[k-j+i]);
                        if(nounit)
                            x[j*incx]=x[j*incx]/conj(A[k]);
                        A+=ldA;
                    }
                }
                else if(uplo==CblasLower)
                {
                    ...
                    for(int j=n-1;j>=0;j--)
                    {
                        ...
                        int im=minadd(j,k,n-1);
                        for(int i=im;i>j;i--)
                            x[j*incx]-=x[i*incx]*conj(A[i-j]);
                        if(nounit)
                            x[j*incx]=x[j*incx]/conj(A[0]);
                    }
                }
            }
        }
            ...
            if(trans==CblasConjTrans)
            {
                if(uplo==CblasLower)
                {
                    ...
                    for(int i=n-1;i>=0;i--)
                    {
                        A-=ldA;
                        if(nounit)
                            x[i*incx]=x[i*incx]/conj(A[k]);
                        int j0=maxsub(i,k);
                        for(int j=i-1;j>=j0;j--)
                            x[j*incx]-=x[i*incx]*conj(A[k-i+j]);
                    }
                }
                else if(uplo==CblasUpper)
                {
                    ...
                    {
                        if(nounit)
                            x[i*incx]=x[i*incx]/conj(A[0]);
                        int jn=minadd(i,k,n-1);
                        for(int j=i+1;j<=jn;j++)
                            x[j*incx]-=x[i*incx]*conj(A[j-i]);
                        A+=ldA;
                    }
                }
            }
